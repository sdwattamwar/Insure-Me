pipeline {
  ...
  stages {
	...
	git 'https://github.com/sdwattamwar/Insure-Me.git'
            }
    }
   stage('Package') {
      steps {
   ...
   stage('Create Docker Image') {
	...
	sh 'docker --version'
     ...
     stage('Docker Login') {
      steps {
        echo 'Docker Login'
	withCredentials([usernamePassword(credentialsId: 'DockerHUbCred', passwordVariable: 'DockerHub_PWD', usernameVariable: 'DockerHub_UN')]) {
